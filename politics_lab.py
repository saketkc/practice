...
def create_voting_dict():
    ...
    for data in voting_data:
        split=data.split()
        voting_list[split[0]]=[int(j) for j in split[3:]]
    return voting_list
...
def most_similar(sen, voting_dict):
    ...
    """
    mins = 00#sum([voting_dict[sen][i]**2 for i in range(0,len(voting_dict[sen]))])
    mins_key=""
    for key in voting_dict.keys():
         if key != sen:
             cal =  policy_compare(sen,key,voting_dict)
             if cal>mins:
                  mins=cal
                  mins_key=key
    return mins_key
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    record=[]
    for key in sen_set:
        record.append(policy_compare(sen,key,voting_dict))
    return sum(record)/len(record)*1.0
...
def find_average_record(sen_set, voting_dict):
    ...
    """
    avg =[]
    for i in range(0,len(voting_dict['Reid'])):
        sums=0
        for key in sen_set:
             sums+=voting_dict[key][i]
        sums=sums/(len(sen_set)*1.0)
        avg.append(sums)
    return avg
...
def bitter_rivals(voting_dict):
    ...
    Example: 
        ...
        ('Fox-Epstein', 'Ravella')
    """
    tuples = [(s1, s2, policy_compare(s1, s2, voting_dict)) for s1 in voting_dict.keys() for s2 in voting_dict.keys() if s1 != s2]
    maxs=1000
    results = ()
    for (a,b,c) in tuples:
        if c<maxs:
